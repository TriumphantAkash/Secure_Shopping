...
public class Client {
	...
	static int []prods = new int[3];
	//********************************************************************************************
	
	static void work(int ECOM) throws IOException, InterruptedException
	{
		//sending client 1 so as to make a session with ECOM1
		System.out.println("Attempting to connect to website : ECOM"+ECOM);
		SendToBroker(cb_session.encrypt(Integer.toString(ECOM)));
		...
		String str_pid_Toamnt = "";
		switch(ECOM) {
		case 1:
			str_pid_Toamnt = ce1_longterm.decrypt(inputFromBroker.readLine());
			break;
		case 2:
		...
		case 3:
			...
			break;
		default:
			System.out.println("[Error] problem in Client Ecommerce decryption\nexiting");
			System.exit(0);
			break;
		}
		...
		if (confirm.equals("Insufficient Funds!"))
		{
			...
			case 3:
				...
				break;
			default:
				System.out.println("[Error] problem in product receive\nexiting");
				System.exit(0);
				break;
			}
